...
static int gcc_debug_mux_sels[] = {
	...
	0x137,		/* gcc_pcie_0_phy_rchng_clk */
	...
	0x13F,		/* gcc_pcie_1_phy_aux_clk */
	0x141,		/* gcc_pcie_1_phy_rchng_clk */
...
static struct clk_debug_mux npu_cc_debug_mux = {
	.priv = &debug_mux_priv,
	.debug_offset = 0x183000,
	.post_div_offset = 0x183004,
	.cbcr_offset = 0x183008,
	.src_sel_mask = 0xFF,
	.src_sel_shift = 0,
	.post_div_mask = 0x3,
	.post_div_shift = 0,
...
static struct clk_hw *debugcc_sm8250_hws[] = {
...
static int clk_debug_sm8250_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_sm8250_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_sm8250_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%d)\n",
				clk_hw_get_name(debugcc_sm8250_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static int __init clk_debug_sm8250_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_sm8250_init);

MODULE_DESCRIPTION("QTI DEBUG CC SM8250 Driver");
