...
static int send_fault_details_qmi(struct qmi_aop_fc_instance *aop_fc)
{
	...
	for (idx = 0; idx < aop_fc->fault_count; idx++) {
		...
		ret = scnprintf(req.payload, PAYLOAD_MAX_LEN, aop_fc->aop_fc_buf[idx].payload);

		if (ret >= PAYLOAD_MAX_LEN) {
			pr_err("Payload is too long\n");
			ret = -E2BIG;
			goto reg_exit;
		}
