...
int gsihal_init(enum gsi_ver gsi_ver, void __iomem *base, phys_addr_t phys_base)
{
	int result = 0;

	GSIDBG("initializing GSI HAL, GSI ver %d, base = %px\n",
		gsi_ver, base);
	...
	if (gsihal_ctx) {
		...
		if (base != gsihal_ctx->base) {
			GSIERR(
				"base address of early init is differnet (%px - %px)\n"
			);
			WARN_ON(1);
		}
		result = -EEXIST;
	...
	if (!base || !phys_base) {
		GSIERR("invalid memory io mapping addr or phys_base\n");
		result = -EINVAL;
		goto bail_err_exit;
	}
	...
	gsihal_ctx->phys_base = phys_base;
