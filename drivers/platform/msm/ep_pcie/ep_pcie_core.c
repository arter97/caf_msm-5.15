...
static void ep_pcie_core_init(struct ep_pcie_dev_t *dev, bool configured)
{
	...
	if (dev->active_config) {
		...
		if (dbi_vf) {
			...
			/* Write DBI_VF register address to PARF_DBI_VF_BASE to activate
                         * access to DBI_VF register.
                         */
	...
	/* Configure inbound IATU prior to BME IRQ of VF
	 * as host can quickly make a BAR access prior to
	 * configuration of inbound IATU causing transaction
...
static void ep_pcie_config_inbound_iatu(struct ep_pcie_dev_t *dev, u32 vf_id)
{
	...
	if (vf_id == PCIE_PHYSICAL_DEVICE) {
		...
		limit = lower + size;
		vf_num = vf_id - 1;
		bar = readl_relaxed(dev->dm_core + PCIE20_SRIOV_BAR(0));
		...
		EP_PCIE_DBG(dev, "MHI PARF vf_id:%d addr:0x%x\n",
			vf_id, PCIE20_PARF_MHI_BASE_ADDR_VFn_LOWER(vf_num));
	}

	/* Bar address is between 4-31 bits, masking 0-3 bits */
	bar &= ~(BIT(0)|BIT(1)|BIT(2)|BIT(3));
...
static irqreturn_t ep_pcie_handle_global_irq(int irq, void *data)
{
	...
	if (dev->sriov_mask) {
		...
		for (i = 0; i < hweight_long(dev->sriov_mask); i++) {
			if ((sriov_irq_status & BIT(i)) && !(dev->sriov_enum & BIT(i))) {
				ep_pcie_notify_vf_bme_event(dev, EP_PCIE_EVENT_LINKUP_VF, i + 1);
				dev->sriov_enum |= BIT(i);
			}
		}
	}
...
int ep_pcie_core_get_msi_config(struct ep_pcie_msi_config *cfg, u32 vf_id)
{
	...
	if (ctrl_reg & BIT(16)) {
		...
		if (ep_pcie_dev.use_iatu_msi) {
			if (ep_pcie_dev.active_config)
				ep_pcie_config_outbound_iatu_entry(&ep_pcie_dev,
						...
						vf_id,
						msi->start + (n * size), EP_PCIE_OATU_UPPER,
						msi->end, lower, upper);
			else
				ep_pcie_config_outbound_iatu_entry(&ep_pcie_dev,
						EP_PCIE_OATU_INDEX_MSI,
						vf_id,
						msi->start + (n * size), 0, msi->end,
						lower, upper);
		}
		...
		if (ep_pcie_dev.active_config || ep_pcie_dev.pcie_edma) {
			...
			cfg->lower = msi->start + (n * size) + (lower & 0xfff);
			cfg->upper = 0;
		}
		cfg->data = data;
...
int ep_pcie_core_trigger_msi(u32 idx, u32 vf_id)
{
	...
	data = readl_relaxed(dbi + PCIE20_MSI_DATA(n));
	ctrl_reg = readl_relaxed(dbi +
					PCIE20_MSI_CAP_ID_NEXT_CTRL(n));
	...
	if (ctrl_reg & BIT(16)) {
		...
		if (ep_pcie_dev.active_config) {
			if (ep_pcie_dev.use_iatu_msi) {
				/*
				 * On targets which SRIOV, configuration of outbound IATU for MSI
				 * host address leads to ECPRI transfers going via outbound IATU
				 * leading to generation on PF BDF always.
