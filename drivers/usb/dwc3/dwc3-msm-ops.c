...
static int entry_trace_dwc3_ctrl_req(struct kretprobe_instance *ri,
				   struct pt_regs *regs)
{
	struct usb_ctrlrequest *ctrl = (struct usb_ctrlrequest *)dwc3_pt_reg(regs, 0);
...
static int entry_trace_dwc3_ep_queue(struct kretprobe_instance *ri,
				   struct pt_regs *regs)
{
	struct dwc3_request *req = (struct dwc3_request *)dwc3_pt_reg(regs, 0);
...
{
	struct dwc3_ep *dep = (struct dwc3_ep *)dwc3_pt_reg(regs, 0);
	unsigned int cmd = dwc3_pt_reg(regs, 1);
	struct dwc3_gadget_ep_cmd_params *param =
			(struct dwc3_gadget_ep_cmd_params *)dwc3_pt_reg(regs, 2);
	int cmd_status = dwc3_pt_reg(regs, 3);
...
{
	struct dwc3_ep *dep = (struct dwc3_ep *)dwc3_pt_reg(regs, 0);
	struct dwc3_trb *trb = (struct dwc3_trb *)dwc3_pt_reg(regs, 1);
...
static int entry_trace_dwc3_event(struct kretprobe_instance *ri,
				   struct pt_regs *regs)
{
	u32 event = dwc3_pt_reg(regs, 0);
	struct dwc3 *dwc = (struct dwc3 *)dwc3_pt_reg(regs, 1);
