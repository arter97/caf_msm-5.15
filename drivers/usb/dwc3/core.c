...
static int dwc3_get_multiport_phys(struct dwc3 *dwc)
{
	...
	for (i = 0; i < dwc->num_ports; i++) {
		snprintf(phy_name, 11, "usb2-port%d", i);
		...
		snprintf(phy_name, 11, "usb3-port%d", i);
...
static int dwc3_read_port_info(struct dwc3 *dwc)
{
	 ...
	 */
	dwc_res.start = dwc->xhci_resources[0].start;
	dwc_res.end = dwc->xhci_resources[0].start +
				DWC3_XHCI_REGS_END;
	dwc_res.flags = dwc->xhci_resources[0].flags;
