...
static LIST_HEAD(gvm_list);
static DEFINE_MUTEX(gvm_list_mutex);
...
struct gvm_dump_ctx {
	...
	struct mutex gvm_mutex;
	struct list_head gvm_ctx_head;
};
...
static void sanitize_gvm_region(struct gvm_dump_ctx *gvm_ctx)
{
	...
	mutex_unlock(&gvm_ctx->gvm_mutex);
}
#else
...
static void cleanup_single_gvm(struct gvm_dump_ctx *gvm_ctx)
{
	...
	list_del(&gvm_ctx->gvm_ctx_head);

	mutex_destroy(&gvm_ctx->gvm_mutex);
	kfree(gvm_ctx);
}

void cleanup_vm_dump_ctx(u16 gvm_id)
{
	struct gvm_dump_ctx *gvm_ctx, *tmp_ctx;

	mutex_lock(&gvm_list_mutex);
	list_for_each_entry_safe(gvm_ctx, tmp_ctx, &gvm_list, gvm_ctx_head) {
		if (gvm_id == gvm_ctx->vmid) {
			sanitize_gvm_region(gvm_ctx);
			cleanup_single_gvm(gvm_ctx);
		}
	}
	mutex_unlock(&gvm_list_mutex);
}
...
static int gvm_debugfs_release(struct inode *inode, struct file *file)
{
	...
	mutex_unlock(&gvm_prv_ctx->gvm_mutex);
...
static int gvm_debugfs_open(struct inode *inode, struct file *file)
{
	...
	mutex_lock(&gvm_prv_ctx->gvm_mutex);
...
static int create_gvm_dump_debugfs(struct device_node *node)
{
	...
	mutex_lock(&gvm_list_mutex);
	list_add(&gvm_ctx->gvm_ctx_head, &gvm_list);
	mutex_unlock(&gvm_list_mutex);
