			 ...
			 bool log_ecn_err)
{
	...
	/* METHOD 2: Check During Packet Processing (Receive Path) */
		...
		u8 protocol)
{
	...
	/* METHOD 2: Check During Packet Processing (Transmit Path) */
	/* try to find matching FMR */

	for (fmr = t->parms.fmrs; fmr; fmr = fmr->next) {
		unsigned int mshift = 32 - fmr->ip4_prefix_len;
		__be32 fmr_prefix_shifted = ntohl(fmr->ip4_prefix.s_addr) >> mshift;
		__be32 pkt_prefix_shifted = ntohl(ip_hdr(skb)->daddr) >> mshift;
		bool match = (fmr_prefix_shifted == pkt_prefix_shifted);
				  ...
				  struct __ip6_tnl_parm *parms)
{
	...
	if (data[IFLA_IPTUN_FMRS]) {
		...
		pr_info("FMR configuration attribute present\n");
		...
		nla_for_each_nested(fmr, data[IFLA_IPTUN_FMRS], rem) {
			...
			err = nla_parse_nested_deprecated(fmrd, IFLA_IPTUN_FMR_MAX,
							  ...
							  NULL);
			if (err) {
				pr_warn("FMR #%d: nla_parse_nested_deprecated failed: %d\n",
					fmr_count, err);
				continue;
			}
			...
			/* Validate FMR before adding */
			if (nfmr->ip6_prefix_len == 0 ||
			    nfmr->ip4_prefix_len == 0 || nfmr->ea_len == 0) {
				pr_warn("FMR #%d: Invalid - required fields are zero\n",
					fmr_count);
				kfree(nfmr);
				continue;
			}
			...
			pr_info("FMR #%d: ip6=%pI6/%u ip4=%pI4/%u ea_len=%u offset=%u\n",
				fmr_count, &nfmr->ip6_prefix, nfmr->ip6_prefix_len,
				&nfmr->ip4_prefix, nfmr->ip4_prefix_len,
				nfmr->ea_len, nfmr->offset);
		}

		/* Summary after parsing all FMRs */
		if (parms->fmrs) {
			int total = 0;
			struct __ip6_tnl_fmr *f;

			for (f = parms->fmrs; f; f = f->next)
				total++;
			pr_info("FMR parsing complete: %d valid FMR(s) configured\n", total);
		} else {
			pr_warn("FMR parsing complete: No valid FMRs configured\n");
		}
	}
}
			   ...
			   struct netlink_ext_ack *extack)
{
	...
	/* METHOD 1: Check if FMR List Exists (Runtime) */
	if (nt->parms.fmrs) {
		struct __ip6_tnl_fmr *fmr;
		int count = 0;

		for (fmr = nt->parms.fmrs; fmr; fmr = fmr->next)
			count++;
		pr_info("FMR enabled for tunnel %s: %d FMR(s) configured\n", nt->parms.name, count);
	}
